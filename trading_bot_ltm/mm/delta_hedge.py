...
class DeltaHedger:
    ...
    def get_inventory_value(self, market_id: str, current_price: float = None) -> float:
        """
        Calcula o valor total do inventory (posições) em um mercado.
        
        Args:
            market_id: ID do mercado
            current_price: Preço atual do mercado (opcional, usa avg_price se None)
        ...
        yes_pos = self.positions[market_id].get("YES", Position(0, 0, 0))
        no_pos = self.positions[market_id].get("NO", Position(0, 0, 0))
        
        # Usar preço atual se fornecido, senão usar preço médio
        price = current_price if current_price is not None else (yes_pos.avg_price if yes_pos.size > 0 else 0.5)
        
        # Valor do inventory = (YES_size * price) + (NO_size * (1 - price))
        # Assumindo que YES + NO = 1.0 (mercado binário)
        yes_value = yes_pos.size * price if yes_pos.size > 0 else 0.0
        no_value = no_pos.size * (1.0 - price) if no_pos.size > 0 else 0.0
