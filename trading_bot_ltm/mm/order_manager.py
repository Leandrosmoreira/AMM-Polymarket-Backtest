...
class OrderManager:
    ...
    async def submit_pre_signed(self, token: str, side: str, price: float) -> Optional[str]:
        ...
        try:
            ...
            if order_id:
                ...
                # Verificar status retornado (MockClobClient pode retornar "filled" imediatamente)
                result_status = result.get("status", "").lower()
                filled_size = result.get("filled_size", result.get("original_size", pre_signed.size))
                ...
                order = Order(
                    ...
                    filled_size=filled_size if result_status == "filled" else 0.0,
                )
                ...
                # Se já está filled, processar imediatamente
                if result_status == "filled":
                    self.update_order_status(order_id, OrderStatus.FILLED, filled_size)
    ...
    async def submit_order(
        ...
        try:
            ...
            if order_id:
                # Verificar status retornado (MockClobClient pode retornar "filled" imediatamente)
                result_status = result.get("status", "").lower()
                filled_size = result.get("filled_size", result.get("original_size", size))
    ...
    def update_order_status(self, order_id: str, status: OrderStatus, filled_size: float = 0):
        """Atualiza status de uma ordem (chamado por WebSocket callback ou após post_order)."""
        if order_id not in self.active_orders:
            return
        ...
        if status == OrderStatus.FILLED:
            ...
            if self.on_fill:
                # Criar objeto compatível com o que on_fill espera
