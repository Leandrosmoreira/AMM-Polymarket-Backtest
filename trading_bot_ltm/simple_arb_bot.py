...
from .ltm_adapter import LTMAdapter
...
class SimpleArbitrageBot:
    ...
    def __init__(self, settings):
        ...
        # LTM (Liquidity Time Model) integration
        self.ltm_adapter = None
        if getattr(settings, 'use_ltm', False):
            try:
                self.ltm_adapter = LTMAdapter(
                    policy_path=getattr(settings, 'ltm_policy_path', None),
                    use_decay_model=getattr(settings, 'ltm_use_decay', True),
                    base_order_size=float(settings.order_size),
                )
    ...
    def get_time_remaining_sec(self) -> float:
    ...
    def check_arbitrage(self, up_book: Optional[dict] = None, down_book: Optional[dict] = None) -> Optional[dict]:
        ...
        When LTM is enabled, uses time-bucket specific thresholds.
        ...
        order_size = float(self.settings.order_size)

        # LTM: Adjust order size based on bucket
        ltm_decision = None
        if self.ltm_adapter:
            time_remaining_sec = self.get_time_remaining_sec()
            # Get preliminary pair cost from best asks for LTM evaluation
            prelim_up = min((p for p, _ in asks_up), default=None) if asks_up else None
            prelim_down = min((p for p, _ in asks_down), default=None) if asks_down else None
            if prelim_up is not None and prelim_down is not None:
                prelim_pair_cost = prelim_up + prelim_down
                ltm_decision = self.ltm_adapter.evaluate(
                    time_remaining_sec=time_remaining_sec,
                    pair_cost=prelim_pair_cost,
                    market_id=self.market_slug,
                )
                # Adjust order size based on LTM
                if ltm_decision.order_size_multiplier > 0:
                    order_size = order_size * min(ltm_decision.order_size_multiplier, 2.0)  # Cap at 2x

        fill_up = self._compute_buy_fill(asks_up, order_size)
        fill_down = self._compute_buy_fill(asks_down, order_size)
        ...
        # Determine threshold - use LTM-adjusted or default
        target_threshold = self.settings.target_pair_cost
        ltm_info = None

        if self.ltm_adapter and ltm_decision:
            # LTM stop trading check
            if not ltm_decision.should_trade:
                if self.settings.verbose:
                    logger.info(f"LTM blocked: {ltm_decision.reason} (bucket={ltm_decision.bucket_index})")
                return None

            # Use LTM-adjusted threshold
            target_threshold = ltm_decision.target_pair_cost
            ltm_info = {
                'bucket_index': ltm_decision.bucket_index,
                'edge_required': ltm_decision.edge_required,
                'size_multiplier': ltm_decision.order_size_multiplier,
                'decay_recommendation': ltm_decision.decay_recommendation,
            }

        # Use <= to avoid missing exact-threshold opportunities due to rounding
        if total_cost <= target_threshold:
            profit = 1.0 - total_cost
            profit_pct = (profit / total_cost) * 100 if total_cost > 0 else 0

            # Use adjusted order size if LTM is enabled
            actual_order_size = order_size if self.ltm_adapter else self.settings.order_size
            investment = total_cost * actual_order_size
            expected_payout = 1.0 * actual_order_size
    ...
    def execute_arbitrage(self, opportunity: dict):
        ...
        # Log LTM info if available
        if 'ltm' in opportunity:
            ltm = opportunity['ltm']
            logger.info("-" * 70)
            logger.info(f"LTM Bucket:           {ltm['bucket_index']} (edge_req={ltm['edge_required']:.2%})")
            logger.info(f"LTM Size multiplier:  {ltm['size_multiplier']:.2f}x")
            if ltm.get('decay_recommendation'):
                logger.info(f"LTM Decay:            {ltm['decay_recommendation']}")

        logger.info("-" * 70)
        logger.info(f"Order size:           {opportunity['order_size']:.0f} shares each side")
    ...
    def show_final_summary(self):
        ...
        # Show LTM stats if available
        if self.ltm_adapter:
            ltm_stats = self.ltm_adapter.get_stats()
            logger.info("-" * 70)
            logger.info("📈 LTM (Liquidity Time Model):")
            logger.info(f"  Trades allowed:               {ltm_stats['trades_allowed']}")
            logger.info(f"  Trades blocked:               {ltm_stats['trades_blocked']}")
            logger.info(f"  Allow rate:                   {ltm_stats['allow_rate']:.1%}")
            if ltm_stats['block_reasons']:
                logger.info("  Block reasons:")
                for reason, count in list(ltm_stats['block_reasons'].items())[:3]:
    ...
    async def monitor(self, interval_seconds: int = 30):
        ...
        if self.ltm_adapter:
            logger.info(f"LTM: ENABLED (buckets={self.ltm_adapter.policy.num_buckets})")
        ...
        try:
            while True:
                ...
                # Snapshot periódico (PnL + Inventory timeline)
                if self.stats_tracker:
                    self.stats_tracker.record_snapshot()
    ...
    async def monitor_wss(self):
        ...
        while True:
            ...
            eval_count = 0
            last_snapshot = 0.0
            snapshot_interval = 1.0  # Snapshot a cada 1 segundo
...
async def main():
    ...
    try:
        ...
        def on_shutdown():
            # Flush and stop fast logger first
            ...
            # Generate EV Analysis Report
            try:
                from .ev_analysis import EVAnalyzer
                analyzer = EVAnalyzer(log_dir="logs")
                analysis = analyzer.analyze()
                if analysis:
                    logger.info("")
                    logger.info(analyzer.format_report(analysis))
            except Exception as e:
                logger.warning(f"Could not generate EV analysis: {e}")
